...
import { getAllCountries, updateCountryStatus } from "../../../redux/actions";
...
const Country = (props) => {
  ...
  const data = !searchCountry
    ? countries
    : countries.filter((a) => {
        debugger;
        return a.name.toLowerCase().includes(searchCountry.toLowerCase());
      });
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/country/edit/${row._id}`}
