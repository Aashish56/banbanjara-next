...
import { getAllEmployee } from "redux/actions";
...
const Employee = (props) => {
  ...
  useEffect(() => {
    try {
      ...
      if (searchName && selectStatus) {
        ...
        filterData = employeeData?.filter((a) => {
          return a.fullName.toLowerCase().includes(searchName?.toLowerCase());
        });
  ...
  const columns = [
    ...
    },
    {
      name: "Phone Number",
      sortable: false,
      selector: (row) => <span className="text-capital">{row?.phoneNumber}</span>
    },
    {
      name: "Email",
      sortable: true,
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/employee/edit/${row._id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/employee/view/${row._id}`}
