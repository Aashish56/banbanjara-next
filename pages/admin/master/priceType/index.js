...
const PriceType = (props) => {
  ...
  useEffect(() => {
    dispatch(getAllPriceType());
  }, []);
  const priceTypeList = useSelector((state) => state.priceType);
  ...
  useEffect(() => {
    try {
      ...
      if (searchName && selectStatus) {
        ...
        filterData = priceTypeList?.filter((a) => {
          return a.name.toLowerCase().includes(searchName?.toLowerCase());
        });
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/master/price_type/edit/${row._id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/master/price_type/view/${row._id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push("/admin/master/price_type/add");
          }}
        >
          {"Add "}
