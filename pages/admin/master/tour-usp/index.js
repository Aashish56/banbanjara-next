...
const TourUSP = (props) => {
  ...
  useEffect(() => {
    dispatch(getAllTourUSP());
  }, []);
  const tourUSPList = useSelector((state) => state.tourUSP);
  ...
  useEffect(() => {
    try {
      ...
      if (searchName && selectStatus) {
        ...
        filterData = tourUSPList?.filter((a) => {
          return a.text.toLowerCase().includes(searchName?.toLowerCase());
        });
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/master/tour-usp/edit/${row._id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/master/tour-usp/view/${row._id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push("/admin/master/tour-usp/add");
          }}
        >
          {"Add "}
