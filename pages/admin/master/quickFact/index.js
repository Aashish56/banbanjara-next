...
import {
  getAllQuickFacts,
  updateQuickFactStatus,
} from "../../../../redux/actions/quickFacts";
...
const QuickFact = (props) => {
  ...
  useEffect(() => {
    dispatch(getAllQuickFacts());
  }, []);
  const quickFactList = useSelector((state) => state.quickFact);

  useEffect(() => {
    setData(quickFactList);
  }, [quickFactList]);
  ...
  useEffect(() => {
    try {
      ...
      if (searchName && selectStatus) {
        ...
        filterData = quickFactList?.filter((a) => {
          return a.text.toLowerCase().includes(searchName?.toLowerCase());
        });
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/master/quick-fact/edit/${row._id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/master/quick-fact/view/${row._id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push("/admin/master/quick-fact/add");
          }}
        >
          {"Add "}
