...
// import { getAllQuickFacts, updateQuickFactStatus } from '../../../../redux/actions/quickFacts';
import {
  getAllThingsToKnows,
  updateThingsToKnowStatus,
} from "../../../../redux/actions/thingsToKnow";
...
const ThingsToKnow = (props) => {
  ...
  useEffect(() => {
    dispatch(getAllThingsToKnows());
  }, []);
  const thingsToKnowList = useSelector((state) => state.thingsToKnow);

  useEffect(() => {
    setData(thingsToKnowList);
    console.log(thingsToKnowList, "data");
  }, [thingsToKnowList]);
  ...
  useEffect(() => {
    try {
      ...
      if (searchName && selectStatus) {
        ...
        filterData = thingsToKnowList?.filter((a) => {
          return a.text.toLowerCase().includes(searchName?.toLowerCase());
        });
  ...
  const columns = [
    ...
    },
    {
      name: "Sub Title",
      sortable: true,
      selector: (row) => <span className="text-capital">{row.subTitle}</span>,
    },
    {
      name: "Display Order",
      sortable: true,
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/master/thingsToKnow/edit/${row._id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/master/thingsToKnow/view/${row._id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push("/admin/master/thingsToKnow/add");
          }}
        >
          {"Add "}
