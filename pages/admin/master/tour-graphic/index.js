...
import {
  getAllTourGraphics,
  updateTourGraphicStatus,
} from "../../../../redux/actions/tourGraphic";

const TourGraphic = (props) => {
  ...
  useEffect(() => {
    dispatch(getAllTourGraphics());
  }, []);
  const tourGraphicList = useSelector((state) => state.tourGraphic);

  useEffect(() => {
    setData(tourGraphicList);
    console.log(tourGraphicList, "data");
  }, [tourGraphicList]);
  ...
  useEffect(() => {
    try {
      ...
      if (searchName && selectStatus) {
        ...
        filterData = tourGraphicList?.filter((a) => {
          return a.text.toLowerCase().includes(searchName?.toLowerCase());
        });
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/master/tour-graphic/edit/${row._id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/master/tour-graphic/view/${row._id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push("/admin/master/tour-graphic/add");
          }}
        >
          {"Add "}
