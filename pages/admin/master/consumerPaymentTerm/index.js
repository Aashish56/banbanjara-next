...
// redux
import {
  getAllConsumerPaymentTerm,
  updateConsumerPaymentTermStatus,
} from "../../../../redux/actions";
// components
...
const ConsumerPaymentTerm = (props) => {
  ...
  useEffect(() => {
    props.setTitle("Consumer Payment Term");
  }, []);
  ...
  useEffect(() => {
    dispatch(getAllConsumerPaymentTerm());
  }, []);
  const consumerPaymentTermList = useSelector(
    (state) => state.consumerPaymentTerm
  );

  useEffect(() => {
    setData(consumerPaymentTermList);
  }, [consumerPaymentTermList]);

  useEffect(() => {
    try {
      var filterData = [];
      if (searchName && selectStatus) {
        filterData = consumerPaymentTermList
          ?.filter((a) => {
        ...
        filterData = consumerPaymentTermList?.filter((a) => {
          return a.name.toLowerCase().includes(searchName?.toLowerCase());
        });
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/master/consumer_payment_term/edit/${row._id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/master/consumer_payment_term/view/${row._id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push("/admin/master/consumer_payment_term/add");
          }}
        >
          {"Add "}
