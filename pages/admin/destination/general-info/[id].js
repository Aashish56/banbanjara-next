...
const LandingPage = (props) => {
  ...
  useEffect(() => {
    if(destinationList?.generalInfo) {
      setData(destinationList?.generalInfo);
      setListing(destinationList?.generalInfo);
    }
  }, [destinationList]);
  ...
  const columns = [
    ...
    },
    {
      name: "Value",
      sortable: true,
      selector: (row) => <span className="text-capital">{row?.value}</span>,
    },
    {
      name: "Action",
      sortable: false,
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/destination/general-info/edit/${row._id}?list=${router.query?.id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/destination/general-info/view/${row._id}?list=${router.query?.id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push(`/admin/destination/general-info/add/${router.query?.id}`);
          }}
        >
          {"Add "}
