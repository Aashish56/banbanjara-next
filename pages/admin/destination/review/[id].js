...
const ReviewPage = (props) => {
  ...
  useEffect(() => {
    if (destinationList?.reviews) {
      setData(destinationList?.reviews);
      setListing(destinationList?.reviews);
    }
  }, [destinationList]);
    ...
    try {
      ...
      if (searchName && selectStatus) {
        ...
        filterData = listing?.filter((a) => {
          return a.comment.toLowerCase().includes(searchName?.toLowerCase());
        });
  ...
  const columns = [
    {
      name: "Rating",
      sortable: true,
      selector: (row) => <span className="text-capital">{row?.rating}</span>,
    },
    {
      name: "Comment",
      sortable: true,
      selector: (row) => <span className="text-capital">
        {row?.comment?.length > 60 ? row?.comment?.substring(0, 60) : row?.comment}
      </span>,
    },
    // {
    //   name: "Status",
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/destination/review/edit/${row._id}?list=${router.query?.id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                ...
                iconclassName="bi bi-pencil-square"
                menuTitle="Manage Destination Page"
              />
            </Dropdown.Item>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/destination/review/view/${row._id}?list=${router.query?.id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push(`/admin/destination/review/add/${router.query?.id}`);
          }}
        >
          {"Add "}
