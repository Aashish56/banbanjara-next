...
const Weather = (props) => {
  ...
  useEffect(() => {
    if (router.query?.id) {
      dispatch(getDestionDetails(router.query?.id));
    }
  }, [router.query?.id]);
  ...
  useEffect(() => {
    if(destinationList?.localWeather) {
      setData(destinationList?.localWeather);
      setListing(destinationList?.localWeather);
    }
  }, [destinationList]);
    ...
    try {
      ...
      if (searchName && selectStatus) {
        ...
        filterData = listing?.filter((a) => {
          return a.month.toLowerCase().includes(searchName?.toLowerCase());
        });
  ...
  const columns = [
    {
      name: "Month",
      sortable: true,
      selector: (row) => <span className="text-capital">{row.month}</span>,
    },
    {
      name: "Min",
      sortable: true,
      selector: (row) => <span className="text-capital">{row.min}</span>,
    },
    {
      name: "Max",
      sortable: true,
      selector: (row) => <span className="text-capital">{row.max}</span>,
    },
    {
      name: "Action",
      sortable: false,
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/destination/weather/edit/${row._id}?list=${router.query?.id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                ...
                iconclassName="bi bi-pencil-square"
                menuTitle="Manage Destination Pages"
              />
            </Dropdown.Item>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/destination/weather/view/${row._id}?list=${router.query?.id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push(`/admin/destination/weather/add/${router.query?.id}`);
          }}
        >
          {"Add "}
