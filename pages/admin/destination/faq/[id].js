...
const FAQPage = (props) => {
  ...
  useEffect(() => {
    if (destinationList?.faq) {
      setData(destinationList?.faq);
      setListing(destinationList?.faq);
    }
  }, [destinationList]);
    ...
    try {
      ...
      if (searchName && selectStatus) {
        ...
        filterData = listing?.filter((a) => {
          return a.question.toLowerCase().includes(searchName?.toLowerCase());
        });
  ...
  const columns = [
    {
      ...
      selector: (row) => <span className="text-capital">
        {row?.question?.length > 60 ? row?.question?.substring(0, 60) : row?.question}
      </span>,
    },
    {
      name: "Answer",
      sortable: true,
      selector: (row) => <span className="text-capital">
        {row?.answer?.length > 60 ? row?.answer?.substring(0, 60) : row?.answer}
      </span>,
    },
    // {
    //   name: "Status",
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/destination/faq/edit/${row._id}?list=${router.query?.id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/destination/faq/view/${row._id}?list=${router.query?.id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push(`/admin/destination/faq/add/${router.query?.id}`);
          }}
        >
          {"Add "}
