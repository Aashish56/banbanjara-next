...
const LandingPage = (props) => {
  ...
  useEffect(() => {
    props.setTitle("Featured Listing Card 1");
  }, []);
  ...
  useEffect(() => {
    if (landingList?.listingCard1) {
      setData(landingList?.listingCard1);
      setListing(landingList?.listingCard1);
    }
  }, [landingList]);
    ...
    try {
      ...
      if (searchName && selectStatus) {
        ...
        filterData = themesList?.filter((a) => {
          return data.isDisable?.toString() === selectStatus;
        });
      } else {
        filterData = listing;
      }
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      selector: (row) => <span className="text-capital">
        {row?.review?.length > 60 ? row?.review?.substring(0, 60) : row?.review}
      </span>,
    },
    {
      name: "Location",
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/landing/listingcard/edit/${row._id}?list=${router.query?.id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/landing/listingcard/view/${row._id}?list=${router.query?.id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push(`/admin/landing/listingcard/add/${router.query?.id}`);
          }}
        >
          {"Add "}
