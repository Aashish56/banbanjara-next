...
const ViewLanding = (props) => {
  ...
  useEffect(() => {
    if (inclusions?.relatedArticles?.length > 0) {
      setData(inclusions.relatedArticles.filter(item => item._id === router.query.id));
    }
  }, [inclusions]);
          ...
          <Table bordered hover>
            ...
            <tr>
              ...
              <tr>
                ...
                <td className="text-capital">{data?.length > 0 && data[0]?.text || ""}</td>
              </tr>
              <tr>
                <th
                  className="text-right"
