...
const LandingPage = (props) => {
  ...
  useEffect(() => {
    props.setTitle("Related Articles");
  }, []);
  ...
  useEffect(() => {
    if (landingList?.relatedArticles) {
      setData(landingList?.relatedArticles);
      setListing(landingList?.relatedArticles);
    }
  }, [landingList]);
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/landing/related-articles/edit/${row._id}?list=${router.query?.id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/landing/related-articles/view/${row._id}?list=${router.query?.id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push(`/admin/landing/related-articles/add/${router.query?.id}`);
          }}
        >
          {"Add "}
