...
const LandingPage = (props) => {
  ...
  useEffect(() => {
    if (landingList?.tourCards1) {
      setData(landingList?.tourCards1);
      setListing(landingList?.tourCards1);
    }
  }, [landingList]);
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/landing/tour-card-1/edit/${row._id}?list=${router.query?.id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/landing/tour-card-1/view/${row._id}?list=${router.query?.id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push(`/admin/landing/tour-card-1/add/${router.query?.id}`);
          }}
        >
          {"Add "}
