...
const LandingPage = (props) => {
  ...
  useEffect(() => {
    if(landingList?.localWeather) {
      setData(landingList?.localWeather);
      setListing(landingList?.localWeather);
    }
  }, [landingList]);
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/landing/weather/edit/${row._id}?list=${router.query?.id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/landing`}
                iconclassName="bi bi-pencil-square"
                menuTitle="Manage Landing Pages"
              />
            </Dropdown.Item>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/landing/weather/view/${row._id}?list=${router.query?.id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push(`/admin/landing/weather/add/${router.query?.id}`);
          }}
        >
          {"Add "}
