...
const LandingPage = (props) => {
  ...
  useEffect(() => {
    props.setTitle("Featured Listing Card 2");
  }, []);
  ...
  useEffect(() => {
    if (landingList?.listingCard2) {
      setData(landingList?.listingCard2);
      setListing(landingList?.listingCard2);
    }
  }, [landingList]);
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      selector: (row) => <span className="text-capital">
        {row?.distance?.length > 60 ? row?.distance?.substring(0, 60) : row?.distance}
      </span>,
    },
    {
      name: "Location",
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/landing/listingcard2/edit/${row._id}?list=${router.query?.id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/landing/listingcard2/view/${row._id}?list=${router.query?.id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push(`/admin/landing/listingcard2/add/${router.query?.id}`);
          }}
        >
          {"Add "}
