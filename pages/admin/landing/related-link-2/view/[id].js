...
const ViewLanding = (props) => {
  ...
  useEffect(() => {
    props.setTitle("View Related Links 2");
  }, []);

  useEffect(() => {
    if (inclusions?.relatedLinks2?.length > 0) {
      setData(inclusions.relatedLinks2.filter(item => item._id === router.query.id));
    }
  }, [inclusions]);
