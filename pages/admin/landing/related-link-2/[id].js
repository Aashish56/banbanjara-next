...
const LandingPage = (props) => {
  ...
  useEffect(() => {
    props.setTitle("Related Links 2");
  }, []);
  ...
  useEffect(() => {
    if(landingList?.relatedLinks2) {
      setData(landingList?.relatedLinks2);
      setListing(landingList?.relatedLinks2);
    }
  }, [landingList]);
  ...
  const columns = [
    {
      name: "Link",
      sortable: true,
      selector: (row) => <span className="text-capital">{row.link}</span>,
    },
    {
      name: "Action",
      sortable: false,
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/landing/related-link-2/edit/${row._id}?list=${router.query?.id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/landing/related-link-2/view/${row._id}?list=${router.query?.id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push(`/admin/landing/related-link-2/add/${router.query?.id}`);
          }}
        >
          {"Add "}
