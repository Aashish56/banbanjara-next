...
const LandingPage = (props) => {
  ...
  useEffect(() => {
    props.setTitle("Explore Near By");
  }, []);
  ...
  useEffect(() => {
    if (landingList?.exploreNearby) {
      setData(landingList?.exploreNearby);
      setListing(landingList?.exploreNearby);
    }
  }, [landingList]);
      ...
      cell: (row) => (
        <Dropdown style={{ border: "1px solid black" }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item>
              <MenuLink
                href={`/admin/landing/explore/edit/${row._id}?list=${router.query?.id}`}
            ...
            <Dropdown.Item>
              <MenuLink
                href={`/admin/landing/explore/view/${row._id}?list=${router.query?.id}`}
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", margin: 10 }}>
        <Button
          ...
          style={{ width: 100 }}
          onClick={() => {
            router.push(`/admin/landing/explore/add/${router.query?.id}`);
          }}
        >
          {"Add "}
