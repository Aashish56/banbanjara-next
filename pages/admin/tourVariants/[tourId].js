...
const Variants = (props) => {
    ...
    values: [
      {
        ...
        minAdult: { type: String },
        maxAdult: { type: String },
        pricePerAdult: { type: String },
        vendorAmountPerAdult: { type: String },

        children: {
          dependant: { type: String },
          independant: { type: String },
        },
        vendorAmountPerChild: { type: String },
      },
    ],
  });
  const initialState = [
  ...
  const variantsRedux = useSelector(
    (state) => state.tours?.currentTour?.variants
  );
  ...
  useEffect(() => {
    const setValues = async () => {
      if (variantsRedux?.length > 0) {
        debugger;
        // variantsRedux.forEach((item) => {

        // });

        // blobUrlToFile

        setVariants(variantsRedux);
        setSelectedInclusions(
          variantsRedux.map((variant) => variant?.inclusions)
        );
      }
    };
    setValues();
  ...
  const onChange = async (e, index) => {
    ...
    if (e.target.name === "image") {
      ...
      setVariants((prev) => {
        prev[index].children[e.target.name] = e.target.value;
        return [...prev];
      });
    } else {
      setVariants((prev) => {
  ...
  const variantJSX = (variant, index, last) => {
    return (
      <Row>
        <Col>
            ...
            <Col md={12} className="mb-0">
              <Select
                ...
                value={data.filter((obj) =>
                  selectedInclusions[index]?.includes(obj.value)
                )} // set selected values
                onChange={(e) => handleInclusionChange(e, index)}
              >
                {selectedInclusions &&
                  selectedInclusions.length > 0 &&
                  selectedInclusions[index] && (
                    <div style={{ marginTop: 20, lineHeight: "25px" }}>
                      <div>
                        <b>Selected Value: </b>{" "}
                        {JSON.stringify(selectedInclusions[index], null, 2)}
                      </div>
                    </div>
                  )}
              </Select>
          ...
          <Accordion defaultActiveKey={variants.length - 1}>
            <Accordion.Item eventKey={index}>
              ...
              <Accordion.Body>
                <Card>
                  <Card.Body>
                    {variantJSX(variant, index, index === variants.length - 1)}
                  </Card.Body>
                </Card>
              </Accordion.Body>
            </Accordion.Item>
