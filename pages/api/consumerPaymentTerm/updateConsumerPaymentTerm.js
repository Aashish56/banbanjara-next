...
async function updateConsumerPaymentTerm(req, res) {
    try {
        ...
        let updateModule = await consumerPaymentTerm.findOneAndUpdate(
            { _id: bodyData._id },
            { $set: setData }
        );
...
export default handler(upload(updateConsumerPaymentTerm,'consumerPaymentTerm','image'));
