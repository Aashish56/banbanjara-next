...
async function getUserWishList(req, res) {
    ...
    try {
        ...
        try {
            ...
            let Data = await wishList.find({ userId: req.body.userId }).sort({ _id: -1 }).populate('propertyId');
            console.log('Data is', Data)
            let result = [];
            for (let x = 0; x < Data.length; x++) {
                let item = Data[x].toObject()
                let propertyId = item.propertyId._id;
                let imageData = await pImage.findOne({ propertyId });
                let FeaturesData = await pFeatures.findOne({ propertyId });
...
export default handler(getUserWishList);