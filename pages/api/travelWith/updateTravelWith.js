...
async function updateTravelWith(req, res) {
    try {
        ...
        let updateModule = await travelWith.findOneAndUpdate(
            { _id: bodyData._id },
            { $set: setData }
        );
...
export default handler(upload(updateTravelWith,'travelWith','image'));